...
class ComfyAPI:
    ...
    def __init__(self, url="localhost:8189", protocol="https"):
        self.server_address = protocol+"://"+url
        self.client_id = INSTANCE_IDENTIFIER
        self.ws_address = f"ws://{url}/ws?clientId={self.client_id}"
        print("WS", self.ws_address)
        self.ws = WebSocket()
    ...
    def find_output_node(json_object): # This method is used to find the node containing the SaveImage class in a prompt
        for key, value in json_object.items():
            if isinstance(value, dict):
                ...
                result = ComfyAPI.find_output_node(value)
                if result:
                    return result
        return None
    ...
    def replace_key_value(json_object, target_key, new_value, class_type_list=None, exclude=True): # This method is used to edit the payload of a prompt
        for key, value in json_object.items():
            ...
            if isinstance(value, dict):
                ...
                ComfyAPI.replace_key_value(value, target_key, new_value, class_type_list, exclude)
            # Recurse sideways (into lists)
            if isinstance(value, list):
                for item in value:
                    if isinstance(item, dict):
                        ComfyAPI.replace_key_value(item, target_key, new_value, class_type_list, exclude)
